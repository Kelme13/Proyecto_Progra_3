...
Enemy_Lip::Enemy_Lip(int hp, Vector2f pos)
{
	this->hp = hp;
...
bool Enemy_Lip::isAlive()
{
	return enemyAlive;
}

void Enemy_Lip::bajarHp(int daño)
{
	hp -= daño;
}

void Enemy_Lip::moverPorImpacto(Retardos a)
{
	retardo = a;
	moviendo_impacto = true;
}

void Enemy_Lip::Draw(RenderTarget& rt) const
{
	rt.draw(spr);
}

void Enemy_Lip::Update(float dt, Vector2f pos_nave)
{
	if (hp <= 0)
	{
